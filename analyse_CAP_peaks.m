function dataPeaks = analyse_CAP_peaks(dataCAP, fnBase, varargin)
...
dataPeaks.edges_fit = edgesTime;
dataPeaks.fit = fitObj;
dataPeaks.peakTime_manual = tt(idxPeaks)';
dataPeaks.peakTime_fit = [fitObj.b1, fitObj.b2, fitObj.b3];
dataPeaks.tt = tt;
dataPeaks.voltage = vv;
...
strPeakTimeManual = sprintf(['Peak_Time_Manual' delimiter '%.3f', ...
    delimiter, '%.3f', delimiter, '%.3f'], dataPeaks.peakTime_manual(1), ...
    dataPeaks.peakTime_manual(2), dataPeaks.peakTime_manual(3));
strPeakTimeFit = sprintf(['Peak_Time_Fit' delimiter '%.3f', ...
    delimiter, '%.3f', delimiter, '%.3f'], dataPeaks.peakTime_fit(1), ...
    dataPeaks.peakTime_fit(2), dataPeaks.peakTime_fit(3));
dataMat = [dataPeaks.tt, dataPeaks.voltage];
...
plot(repmat((dataPeaks.peakTime_manual').*ones(3,1), 1, 2), yLims, ':', ...
    'Color', colOrder(1, :))
plot(repmat((dataPeaks.peakTime_fit').*ones(3,1), 1, 2), yLims, ':', ...
    'Color', colOrder(2, :))
